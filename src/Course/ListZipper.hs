...
findLeft p (ListZipper l x r) = case break p l of
  (_, Nil) -> MLZ Empty
  (right, y :. res) -> MLZ . Full $ ListZipper res y (reverse right ++ pure x ++ r)
...
findRight p (ListZipper l x r) = case break p r of
  (_, Nil) -> MLZ Empty
  (left, y :. res) -> MLZ . Full $ ListZipper (reverse left ++ pure x ++ l) y res
...
last (x :. Nil) = x
last (_ :. xs) = last xs
last _ = error "last on empty list!"

moveLeftLoop ::
  ListZipper a ->
  ListZipper a
moveLeftLoop (ListZipper Nil x r) = ListZipper (drop 1 . reverse $ (x :. r)) (last r) Nil
...
moveRightLoop ::
  ListZipper a ->
  ListZipper a
moveRightLoop (ListZipper l x Nil) = ListZipper Nil (last l) (drop 1 . reverse $ (x :. l))
...
moveLeft ::
  ListZipper a ->
  MaybeListZipper a
moveLeft lz = case moveLeftLoop lz of
  ListZipper _ _ Nil -> MLZ Empty
  x -> MLZ . Full $ x
...
moveRight ::
  ListZipper a ->
  MaybeListZipper a
moveRight lz = case moveRightLoop lz of
  ListZipper Nil _ _ -> MLZ Empty
  x -> MLZ . Full $ x
...
swapLeft (ListZipper (l :. ls) x r) = MLZ . Full $ ListZipper (x :. ls) l r
...
swapRight (ListZipper l x (r :. rs)) = MLZ . Full $ ListZipper l r (x :. rs)
...
moveRightN ::
  ...
  ListZipper a ->
  MaybeListZipper a
moveRightN 0 lz = MLZ . Full $ lz
moveRightN n lz
  | n < 0 = moveLeftN (negate n) lz
  | otherwise = case moveRight lz of
      MLZ (Full lz') -> moveRightN (n - 1) lz'
      x -> x
...
moveLeftN' n lz
  | n < 0 = moveRightN' (negate n) lz
  | otherwise = moveLeftAux n 0 lz
  where
    moveLeftAux 0 _ z = Right z
    moveLeftAux toMove totalMoves z = case moveLeftN 1 z of
      MLZ Empty -> Left totalMoves
      MLZ (Full x) -> moveLeftAux (toMove - 1) (totalMoves + 1) x
...
moveRightN' n lz
  | n < 0 = moveLeftN' (negate n) lz
  | otherwise = moveRightAux n 0 lz
  where
    moveRightAux 0 _ z = Right z
    moveRightAux toMove totalMoves z = case moveRightN 1 z of
      MLZ Empty -> Left totalMoves
      MLZ (Full x) -> moveRightAux (toMove - 1) (totalMoves + 1) x
...
nth ::
  ...
  ListZipper a ->
  MaybeListZipper a
nth n = (moveRightN n -<<) . fromList . toList
...
deletePullLeft (ListZipper (l :. ls) _ r) = MLZ . Full $ ListZipper ls l r
...
insertPushLeft ::
  ...
  ListZipper a ->
  ListZipper a
insertPushLeft x (ListZipper ls l r) =
  ListZipper (l :. ls) x r
...
insertPushRight ::
  ...
  ListZipper a ->
  ListZipper a
insertPushRight x (ListZipper l r rs) =
  ListZipper l x (r :. rs)
...
instance Extend ListZipper where
  (<<=) f lz =
    ListZipper (unfoldr leftMoves lz) (f lz) (unfoldr rightMoves lz)
    where
      leftMoves l =
        case moveLeft l of
          MLZ Empty -> Empty
          MLZ (Full x) -> Full (f x, x)
      rightMoves r =
        case moveRight r of
          MLZ Empty -> Empty
          MLZ (Full x) -> Full (f x, x)
...
instance Extend MaybeListZipper where
  ...
  (<<=) f mlz =
    MLZ . Full $ ListZipper
    (unfoldr leftMoves mlz)
    (f mlz)
    (unfoldr rightMoves mlz)

    where leftMoves l = case moveLeft -<< l of
                          MLZ Empty -> Empty
                          y -> Full (f y, y)
          rightMoves r = case moveRight -<< r of
                          MLZ Empty -> Empty
                          y -> Full (f y, y)
...
instance Traversable ListZipper where
  ...
  traverse akb (ListZipper l a r) = ListZipper <$> traverse akb l <*> akb a <* traverse akb l <*> traverse akb r -- have to make sure traverse l gets run before akb a
...
instance Traversable MaybeListZipper where
  ...
  traverse f (MLZ (Full x)) = isZ <$> traverse f x
