...
instance Functor k => Functor (StateT s k) where
  ...
  (<$>) ab ska = StateT $ \s -> let
    res = runStateT ska s
    in (\(a,newState) -> (ab a, newState)) <$> res
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<*>) skab ska = StateT $ \s -> do
    (kab, s1) <- runStateT skab s
    (ka, s2) <- runStateT ska s1
    pure (kab ka, s2)
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (=<<) askb ska = StateT $ \s -> do
    (a, s1) <- runStateT ska s
    runStateT (askb a) s1
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = join (flip evalT S.empty $ do
                          filtered <- filtering uniq xs
                          let res = sequence . (check100 <$>) $ filtered
                          pure res)
  where check100 x
          | x <= 100 = Full x
          | otherwise = Empty
...
-- check100 = foldRight (\x acc -> if x <= 100 then (x :.) <$> acc else Empty) (Full Nil)
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<*>) okab oka = OptionalT $ do
    koab <- runOptionalT okab
    onFull (\f ->
      runOptionalT oka >>= \koa ->
        onFull (pure . Full . f) koa ) koab
...
distinctG xs = join $ flip evalT S.empty $ do
                  filtered <- filtering uniq xs -- Filter the unique numbers
                  let log = sequence <$> (sequence . map logNumber) xs -- generate a log based on the numbers passed to
                  pure $ (filtered <$) <$> log -- Place the filtered list into the log return list if it returns a valid log
  where logNumber x
          | x > 100 = log1 ("aborting > 100: " ++ listh (show x)) Empty
          | even x  = log1 ("even number: " ++ listh (show x)) (Full x)
          | otherwise = pure $ Full x
        uniq x = do seen <- getT
                    putT (S.insert x seen)
                    pure (S.notMember x seen)
