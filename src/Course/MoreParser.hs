...
hex ::
  Parser Char
hex = do
  h <- readHex <$> thisMany 4 (satisfy isHexDigit)
  case chr <$> h of
    Full valid -> pure valid
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby a sep = sepby1 (tok a) (tok sep) ||| pure Nil
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll cbs = satisfy (and . sequence cbs)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny cbs = satisfy (or . sequence cbs)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma o c pa = betweenCharTok o c (sepby pa commaTok) 